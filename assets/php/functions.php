...
{
    $uid = $_GET['blockid'];
    $query="UPDATE users SET ac_status=2 WHERE id=$uid";
    mysqli_query($db,$query);
    header("Location: http://localhost/SocialMedia/?dashboard#unverifiedcustomers");
}

if(isset($_GET['unblockid']))
{
    $uid = $_GET['unblockid'];
    $query="UPDATE users SET ac_status=1 WHERE id=$uid";
    mysqli_query($db,$query);
    header("Location: http://localhost/SocialMedia/?dashboard#customers");
}
...
function isContactRegistered($contact){
    global $db;
    $query="SELECT count(*) as row FROM users WHERE contact='$contact'";
    $run=mysqli_query($db,$query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}
...
$response['status']=true;
    ...
    if(!$form_data['designation']){
        $response['msg']="designation is not given";
        $response['status']=false;
        $response['field']='designation';
    }
    if(!$form_data['contact']){
        $response['msg']="contact is not given";
        $response['status']=false;
...
function createUser($data){
 ...
 $query = "INSERT INTO users(first_name,last_name,gender,email,username,password,designation,address,contact) ";
 $query.="VALUES ('$first_name','$last_name',$gender,'$email','$username','$password','$designation','$address','$contact')"; 
 return mysqli_query($db,$query);
}

//function for verify contact
function verifyContact($contact){
    global $db;
    $query="UPDATE users SET ac_status=1 WHERE contact='$contact'";
    return mysqli_query($db,$query);
}

//function for verify email
function resetPassword($contact,$password){
    global $db;
    $password=md5($password);
    $query="UPDATE users SET password='$password' WHERE contact='$contact'";
    return mysqli_query($db,$query);
}
...
if($imagedata['name']){
    ...
    $profile_pic=", profile_pic='$image_name'";
        ...
        $query = "UPDATE users SET first_name = '$first_name', last_name='$last_name',username='$username', bio='$bio', password='$password' $profile_pic WHERE id=".$_SESSION['userdata']['id'];
return mysqli_query($db,$query);
...
$user_id = $_SESSION['userdata']['id'];
   ...
   function createFeedback($data){
    global $db;
    $feedback = mysqli_real_escape_string($db,$data['feedback']);
