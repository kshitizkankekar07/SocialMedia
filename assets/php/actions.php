...
if(isset($_GET['login'])){
    ...
    if($response['status']){
     ...
     $_SESSION['code']=$code = rand(1111,9999);
     sendCode($response['user']['contact'],'Verify Your Contact',$code);
     }
    ...
    if(isset($_GET['resend_code'])){
            ...
            $_SESSION['code']=$code = rand(1111,9999);
            sendCode($_SESSION['userdata']['contact'],'Verify Your Contact',$code);
    ...
    if(isset($_GET['verify_email'])){
       ...
       $code = $_SESSION['code'];
       if($code==$user_code){
       if(verifyContact($_SESSION['userdata']['contact'])){
        header('location:../../');
       }else{
           ...
           if(!$_POST['code']){
            $response['msg']='enter 4 digit code !';
...
if(isset($_GET['forgotpassword'])){
    if(!$_POST['email']){
        $response['msg']="enter your Contact Number !";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');

    }elseif(!isContactRegistered($_POST['email'])){
        $response['msg']="Contact is not registered";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');
          ...
          $_SESSION['forgot_email']=$_POST['email'];
           $_SESSION['forgot_code']=$code = rand(1111,9999);
...
if(isset($_GET['addfeedback'])){
        if(createFeedback($_POST)){
        header('location:../../?Feedback_added');
        }else{
            echo "<script>alert('somethihng is wrong')</script>";
        }
    }
